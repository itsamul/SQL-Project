...
SELECT first_name, LENGTH(first_name) AS length_first_name
FROM employees
WHERE LENGTH(first_name) > 5 AND gender = 'M'
ORDER BY length_first_name ASC;
...
SELECT first_name, last_name, gender, REPLACE(gender, 'M', 'Male') AS emp_gender_male
FROM employees;

-- 4.2: Change F to Female in the gender column of the employees table
SELECT first_name, last_name, gender, REPLACE(gender, 'F', 'Female') AS emp_gender_female
FROM employees;
...
SELECT bracket_cust_id, TRIM(bracket_cust_id, '()') AS cleaned_customer_id
FROM customers;
...
SELECT customer_id, customer_name, SUBSTRING(customer_id FOR 2) AS customer_group
FROM customers;
...
SELECT customer_id, customer_name, SUBSTRING(customer_id FROM 4 FOR 5) AS customer_number
FROM customers
WHERE SUBSTRING(customer_id FOR 2) = 'AB';

-- 7.5: Retrieve the year of birth for all employees
SELECT emp_no, birth_date, SUBSTRING(CAST(birth_date AS CHAR(10)) FOR 4) AS birth_year
FROM employees;
